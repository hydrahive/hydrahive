...
function statusIcon(status: string, hung: boolean) {
  ...
  switch (status) {
    ...
    default:           return <Square className="w-4 h-4 text-muted-foreground" />;
  }
}
          ...
          <button
            ...
            className={cn(
              ...
              confirmStop
                ...
                : "border-border text-muted-foreground hover:bg-red-50 hover:border-red-400 hover:text-red-600 dark:hover:bg-red-900/30",
...
function AgentDetailPanel({
  ...
  return (
    <>
      ...
      {/* Panel */}
      <div className="fixed inset-y-0 right-0 w-[calc(100%-2rem)] sm:max-w-lg bg-background shadow-2xl z-50 flex flex-col">
        ...
        <div className={cn(
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              ...
              <div className="text-xs text-muted-foreground">{agent.id} · {agent.type}</div>
            </div>
          </div>
          <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-muted text-muted-foreground" aria-label="Close detail panel">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="px-5 py-4 border-b border-border shrink-0 space-y-3">
          ...
          <div className="grid grid-cols-3 gap-3 text-xs">
            <div className="bg-muted rounded-lg px-3 py-2">
              <div className="text-muted-foreground mb-0.5">{t("activity.detail.model")}</div>
              <div className="font-medium truncate">{agent.model ?? "—"}</div>
            </div>
            <div className="bg-muted rounded-lg px-3 py-2">
              <div className="text-muted-foreground mb-0.5">{t("activity.detail.heartbeat")}</div>
              <div className={cn("font-medium", hung && "text-orange-500")}>
                {hbAge.toFixed(0)}s
              </div>
            </div>
            <div className="bg-muted rounded-lg px-3 py-2">
              <div className="text-muted-foreground mb-0.5">{t("activity.detail.restarts")}</div>
        ...
        <div className="flex items-center gap-2 px-5 py-2.5 border-b border-border shrink-0">
          ...
          <span className="text-xs font-medium text-muted-foreground">{t("activity.detail.logs")}</span>
          {logsLoading && <Loader2 className="w-3 h-3 animate-spin text-muted-foreground ml-auto" />}
        </div>
        <div className="flex-1 overflow-y-auto px-4 py-3 font-mono text-xs bg-gray-950 text-green-400 leading-relaxed">
          {logs.length === 0 && !logsLoading && (
            <span className="text-muted-foreground italic">{t("activity.detail.noLogs")}</span>
          )}
          {logs.map((line, i) => (
  ...
  return (
    <div className="p-6 space-y-6 max-w-7xl mx-auto">
      ...
      {!loading && !error && data.length === 0 && (
        <div className="text-center text-muted-foreground py-16">{t("activity.noAgents")}</div>
      )}
