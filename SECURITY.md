...
OctopOS is currently in active development. Security fixes are applied to the latest version on `main`.
...
OctopOS is designed for self-hosted, private-network deployment. Key security properties:
...
- **Secrets externalized** — All credentials live in `/etc/octopos/` (not in the repo)
- **Project isolation** — Each project runs as a dedicated Linux user

## Deployment Recommendations

- Run OctopOS behind a reverse proxy (nginx) with TLS
- Do not expose port 8765 (Core API) directly — use the nginx proxy on port 80/443
- Keep `/etc/octopos/` readable only by the `octopos` user (`chmod 700`)
- Rotate the JWT secret (`/etc/octopos/jwt_secret`) if you suspect compromise — this invalidates all active sessions
- Use a dedicated non-root system user (`octopos`) as the service account
